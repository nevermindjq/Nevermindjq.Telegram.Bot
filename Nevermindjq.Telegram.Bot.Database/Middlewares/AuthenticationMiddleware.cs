...
public class AuthenticationMiddleware<TUser, TRole, TRedirect>(IRepository<TUser> users, IUpdateResolver resolver) : ICommandMiddleware<IAuthenticated<TUser, TRole>>
	...
	public async Task<IMiddlewareResponse> HandleAsync(Update update, IAuthenticated<TUser, TRole> command) {
		var user = await users.GetAsync(resolver.UserId(update));
